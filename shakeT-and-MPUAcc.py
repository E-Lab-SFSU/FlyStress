#!/usr/bin/env python3
"""
FlyStress combined MPU6050 + Ender 3 shake test logger

What this script does:
1. Connects to the MPU6050 over I2C.
2. Wakes the MPU6050 and configures the accelerometer range.
3. Calibrates the MPU while the printer/bed is still.
4. Starts the printer shake test in a background thread.
5. Samples MPU accelerometer + gyroscope data during the shake test.
6. Saves all readings to a CSV file.
7. Creates a line plot PNG from the saved acceleration data.

Run from the Raspberry Pi terminal:
    python3 flystress_combined_shake_mpu.py
...
VELOCITY = 220.0           # mm/s
ACCELERATION = 2000.0      # mm/s^2. Ender 3-safe default; adjust if desired.
JERK = 20.0                # mm/s
DURATION = 20.0            # seconds of shaking

# Positioning
CENTER_Y = 25.0            # safe middle of Y travel
HOME_FIRST = True
RAISE_Z_FIRST = True
Z_LIFT = 50.0              # mm
...
def calibrate_mpu(mpu, samples, sample_rate_hz, accel_lsb_per_g):
    """Return accel and gyro offsets while the MPU is stationary.

    Assumes Z axis is pointed upward, so expected Z acceleration is +1g.
    If your MPU is mounted in another orientation, you can still use the data,
    but the Z offset assumption may need adjusting.
    """
...
def run_shake_test(stop_event):
    ...
    try:
        ...
        # Marlin commonly resets on USB connect.
        time.sleep(3)
...
def write_metadata(writer, offsets, accel_lsb_per_g, gyro_lsb_per_dps):
    writer.writerow(["# FlyStress combined shake + MPU6050 log"])
...
def log_mpu_during_shake(mpu, offsets, csv_path, accel_lsb_per_g, gyro_lsb_per_dps, stop_event):
    ...
    with open(csv_path, "w", newline="") as csv_file:
        ...
        write_metadata(writer, offsets, accel_lsb_per_g, gyro_lsb_per_dps)
        writer.writerow([
            "t_sec", "timestamp",
