...
class MPUReader(QThread):
    ...
    def run(self):
        if not self.settings.enabled:
            self.status_ready.emit("MPU disabled.")
            return

        if SMBus is None:
            self.error_ready.emit("smbus2 is not installed. Install with: pip install smbus2")
            return

        try:
            self.bus = SMBus(self.settings.bus_number)
            time.sleep(1)
            self.bus.write_byte_data(self.settings.address, self.POWER_MGMT_1, 0x00)
            self.running = True
            self.status_ready.emit("MPU connected and streaming.")
            ...
            self.error_ready.emit(f"MPU error: {e}")
            ...
            if self.bus:
                try:
                    ...
                    pass
            self.status_ready.emit("MPU stopped.")
    ...
    def read_sample(self):
        try:
            ...
            gyro_x_raw = self.read_word(0x43)
            gyro_y_raw = self.read_word(0x45)
            gyro_z_raw = self.read_word(0x47)

            gyro_x_deg_s = gyro_x_raw / 131.0
            gyro_y_deg_s = gyro_y_raw / 131.0
            gyro_z_deg_s = gyro_z_raw / 131.0

            accel_x_raw = self.read_word(0x3B)
            accel_y_raw = self.read_word(0x3D)
            accel_z_raw = self.read_word(0x3F)
            ...
            self.error_ready.emit(f"MPU read error: {e}")
            return None
    ...
    def start_recording(self, csv_path):
        try:
            ...
            self.recording = True
            self.status_ready.emit(f"MPU saving to: {csv_path}")
        except Exception as e:
            self.error_ready.emit(f"Could not start MPU recording: {e}")

    def stop_recording(self):
        self.recording = False
        self.close_csv()
        self.status_ready.emit("MPU recording stopped.")
    ...
    def write_sample(self, sample):
        self.csv_writer.writerow([
            ...
            sample["gyro_x_deg_s"], sample["gyro_y_deg_s"], sample["gyro_z_deg_s"],
            sample["x_rotation"], sample["y_rotation"]
        ])
        self.csv_file.flush()
...
class MPULiveWindow(QWidget):
    def __init__(self):
        ...
        layout = QVBoxLayout()
        self.display = QLabel("Waiting for MPU data...")
        self.display.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignTop)
        self.display.setStyleSheet("font-family: Courier; font-size: 13px;")
    ...
    def update_sample(self, sample):
        self.display.setText(
            ...
            "Accelerometer\n"
            f"X raw: {sample['accel_x_raw']:6d}   X g: {sample['accel_x_g']:.4f}\n"
            f"Y raw: {sample['accel_y_raw']:6d}   Y g: {sample['accel_y_g']:.4f}\n"
            f"Z raw: {sample['accel_z_raw']:6d}   Z g: {sample['accel_z_g']:.4f}\n\n"
            "Gyroscope\n"
            f"X raw: {sample['gyro_x_raw']:6d}   X deg/s: {sample['gyro_x_deg_s']:.4f}\n"
            f"Y raw: {sample['gyro_y_raw']:6d}   Y deg/s: {sample['gyro_y_deg_s']:.4f}\n"
            f"Z raw: {sample['gyro_z_raw']:6d}   Z deg/s: {sample['gyro_z_deg_s']:.4f}\n\n"
...
class ExperimentSettings:
    def __init__(self):
        ...
        # MPU settings: the MPU reader starts automatically with an experiment
        # and records until the experiment is stopped.
        self.enable_mpu = True
        self.mpu_bus_number = 1
        self.mpu_address = 0x68
        self.mpu_sample_interval_ms = 100
...
class FlyShakerGUI(QWidget):
    def __init__(self):
        ...
        self.mpu_live_window = MPULiveWindow()
        ...
        self.enable_mpu = QCheckBox("Open MPU window and save MPU during experiment")
        self.enable_mpu.setChecked(True)

        self.mpu_interval = QSpinBox()
        self.mpu_interval.setRange(10, 5000)
        self.mpu_interval.setValue(100)
        ...
        row += 1
        controls.addWidget(self.enable_mpu, row, 0, 1, 2)
        row += 1
        controls.addWidget(QLabel("MPU Sample ms"), row, 0)
        controls.addWidget(self.mpu_interval, row, 1)
        row += 1

        # Space reserved for later shake-control widgets.
        # Example later: Shake cycles, jerk, distance, acceleration, velocity, AFS_SEL.
    ...
    def collect_settings(self):
        ...
        s.enable_mpu = self.enable_mpu.isChecked()
        s.mpu_sample_interval_ms = self.mpu_interval.value()
    ...
    def start_mpu_for_experiment(self, settings):
        if not settings.enable_mpu:
            return

        output_root = Path(settings.output_dir)
        output_root.mkdir(parents=True, exist_ok=True)
        csv_path = output_root / "mpu_data.csv"

        mpu_settings = MPUSettings()
        mpu_settings.enabled = True
        mpu_settings.bus_number = settings.mpu_bus_number
        mpu_settings.address = settings.mpu_address
        mpu_settings.sample_interval_ms = settings.mpu_sample_interval_ms

        self.mpu_reader = MPUReader(mpu_settings)
        self.mpu_reader.sample_ready.connect(self.update_mpu_window)
        self.mpu_reader.status_ready.connect(self.update_status)
        self.mpu_reader.error_ready.connect(self.update_status)
        self.mpu_reader.start()
        self.mpu_live_window.show()

        # Give the thread a moment to open the bus before opening the CSV.
        # If the bus fails, the error will appear in the GUI status.
        self.mpu_reader.msleep(100)
        self.mpu_reader.start_recording(str(csv_path))

    def stop_mpu_for_experiment(self):
        if self.mpu_reader:
            self.mpu_reader.stop_recording()
            self.mpu_reader.stop()
            self.mpu_reader = None
        self.mpu_live_window.hide()

    def update_mpu_window(self, sample):
        self.mpu_live_window.update_sample(sample)
    ...
    def start_experiment(self):
        ...
        self.worker.start()
        self.start_mpu_for_experiment(settings)

        # Space reserved for later shake protocol start.
        # Example later:
        # self.start_shake_protocol(settings)

    def stop_experiment(self):
        # Space reserved for later shake protocol stop.
        # Example later:
        # self.stop_shake_protocol()

        if self.worker:
            self.worker.stop()
        self.stop_mpu_for_experiment()
        self.stop_button.setEnabled(False)
