...
class FlyStressApp:
    ...
    def __init__(self, root, defaults: StartupDefaults):
        ...
        self.tools_plot_canvas = None
        self.tools_plot_frame = None
        self.tools_stats_text = None
    ...
    def tools_page(self):
        ...
        self.tools_plot_frame = self.make_frame(display_frame)
        self.tools_plot_frame.pack(side="left", fill="both", expand=True, padx=(0, 10))

        stats_frame = self.make_label_frame(display_frame, "Box Plot Stats")
        stats_frame.pack(side="right", fill="y", padx=(10, 0))

        self.tools_stats_text = tk.Text(
            stats_frame,
            width=42,
            height=24,
            bg="white",
        ...
        self.tools_stats_text.pack(fill="both", expand=True, padx=8, pady=8)
        self.tools_stats_text.insert(tk.END, "Box plot statistics will appear here.\n")

        self.make_label(
            self.tools_plot_frame,
    ...
    def clear_tools_plot(self):
        if self.tools_plot_frame is None:
            return
        for widget in self.tools_plot_frame.winfo_children():
            widget.destroy()
        self.tools_plot_canvas = None

    def display_figure_in_tools(self, fig):
        if self.tools_plot_frame is None:
            plt.close(fig)
            return
        self.clear_tools_plot()
        self.tools_plot_canvas = FigureCanvasTkAgg(fig, master=self.tools_plot_frame)
        self.tools_plot_canvas.get_tk_widget().pack(fill="both", expand=True)
        self.tools_plot_canvas.draw()
    ...
    def axis_statistics(self, values):
        return {
            "min": min(values),
            "q1": self.percentile_value(values, 25),
            "median": self.percentile_value(values, 50),
            "q3": self.percentile_value(values, 75),
            "max": max(values),
        }

    def write_stats_to_text_box(self, stats, stats_csv_path):
        if self.tools_stats_text is None:
            return
        self.tools_stats_text.delete("1.0", tk.END)
        self.tools_stats_text.insert(tk.END, "Box Plot Statistics\n")
        self.tools_stats_text.insert(tk.END, f"Saved to:\n{stats_csv_path}\n\n")
        self.tools_stats_text.insert(tk.END, "Axis      Min       Q1    Median       Q3      Max\n")
        self.tools_stats_text.insert(tk.END, "-----------------------------------------------\n")
        for axis in ("x", "y", "z"):
            s = stats[axis]
            self.tools_stats_text.insert(
                tk.END,
                f"{axis.upper():<5} {s['min']:8.4f} {s['q1']:8.4f} {s['median']:8.4f} {s['q3']:8.4f} {s['max']:8.4f}\n"
            )
    ...
    def load_csv_plot(self):
        ...
        try:
            ...
            ax.set_title("CSV Line Plot", fontname=self.FONT, fontsize=18, fontweight="bold", color=self.TEXT_COLOR)
            ...
            png_path = os.path.splitext(path)[0] + "_line_plot.png"
            fig.savefig(png_path, dpi=300)
            self.display_figure_in_tools(fig)

            self.set_status(f"Loaded CSV line plot. Saved PNG: {png_path}")
    ...
    def make_boxplot(self):
        ...
        try:
            ...
            stats = {
                "x": self.axis_statistics(ax_g),
                "y": self.axis_statistics(ay_g),
                "z": self.axis_statistics(az_g),
            }

            stats_csv_path = os.path.splitext(path)[0] + "_box_stats.csv"
            with open(stats_csv_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["axis", "min", "q1", "median", "q3", "max"])
                for axis in ("x", "y", "z"):
                    s = stats[axis]
                    writer.writerow([
                        axis,
            ...
            try:
                ax.boxplot([ax_g, ay_g, az_g], tick_labels=["ax_g", "ay_g", "az_g"])
            except TypeError:
            ...
            png_path = os.path.splitext(path)[0] + "_box_plot.png"
            fig.savefig(png_path, dpi=300)

            self.display_figure_in_tools(fig)
            self.write_stats_to_text_box(stats, stats_csv_path)

            self.set_status(f"Created box plot. PNG: {png_path} | Stats CSV: {stats_csv_path}")
